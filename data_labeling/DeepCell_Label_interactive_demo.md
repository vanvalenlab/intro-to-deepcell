...
1. Download the tissue imaging .npz file from our [S3 bucket](https://caliban-input.s3.us-east-2.amazonaws.com/janelia_demo/HeLa-S3_janelia_demo_version.npz). This file is prepopulated with labels.
...
## Whole-label mode (TODO: fold into above examples)

### Select labels in whole-label mode

Click on a label to select it. Some actions require just one selected label, while others require two selected labels.
...
### Flood regions with a label

Press <kbd>G</kbd> to use the Flood tool. Select the foreground label that you want to flood with and select the background label you want to

Remember you can always <kbd>Shift</kbd> + Double Click to select the foreground and <kbd>Shift</kbd> + Click to select the background. You can also simply Click on an unselected label to make it the background while using the Flood tool.
...
### Split labels with Watershed

Press <kbd>W</kdb> to use the Watershed tool.
...
## Threshold the brightest pixels in a box
